...
create table if not exists public.attendance (
  ...
  subject_id uuid not null references public.subjects(id) on delete cascade,
  date date not null,
  status text not null check (status in ('present', 'absent', 'holiday', 'exam')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (user_id, subject_id, date)
);
-- Safe migration: if this project previously used the wider status set
-- (non_working / medical_leave / od), fold those into 'holiday' (they never
-- affected attendance %) before tightening the constraint.
update public.attendance set status = 'holiday' where status in ('non_working', 'medical_leave', 'od');
alter table public.attendance drop constraint if exists attendance_status_check;
alter table public.attendance add constraint attendance_status_check check (status in ('present', 'absent', 'holiday', 'exam'));
