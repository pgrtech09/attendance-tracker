...
import { subjectWiseStats, averageSubjectPercentage, pooledHourPercentage, remainingWorkingDaysInRange, targetProjection, todayLocalISO } from './attendance-calc.js';
...
function renderStatCards(stats, holidays, settings, target) {
  ...
  const pooled = pooledHourPercentage(stats);
  const pooledEl = document.getElementById('statOverallPooled');
  if (pooledEl) {
    pooledEl.textContent = `${pooled.toFixed(1)}% by hrs`;
    pooledEl.title = 'Extra display: present hours ÷ total hours × 100, pooled across every subject';
  }
