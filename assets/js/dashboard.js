...
import { tallyRecords, subjectWiseStats, targetProjection, monthlyBreakdown, remainingWorkingDaysInRange } from './attendance-calc.js';
...
function renderStatCards(subjects, attendance, settings, holidays, timetable) {
  ...
  document.getElementById('statOverall').textContent = `${overall.percentage.toFixed(1)}%`;
  document.getElementById('statOverallSub').textContent = `${overall.present} present of ${overall.workingTotal} classes`;
  ...
  document.getElementById('statTargetSub').textContent = proj.onTrack
    ? `On track — can miss ${proj.bunkable} more`
    : `Need ${proj.classesRequired} more present`;
...
function renderCharts(subjects, attendance) {
  ...
  pieChart = new Chart(pieCtx, {
    type: 'doughnut',
    data: {
      labels: ['Present', 'Absent', 'Holiday', 'Exam', 'Medical Leave', 'OD', 'Non-Working'],
      datasets: [{
        data: [overall.present, overall.absent, overall.holiday, overall.exam, overall.medical_leave, overall.od, overall.non_working],
        backgroundColor: ['#2F9E67', '#D64545', '#4C7EA8', '#7D5BA6', '#3FAEA1', '#B98B2E', '#8A93A6'],
        borderWidth: 0
      }]
    },
