...
import { initShell, toast, toggleTheme, localISODate } from './app.js';
...
const todayISOForBulk = localISODate();
...
document.getElementById('bulkMarkBtn').addEventListener('click', async () => {
  ...
  const { data: fullTimetable } = await supabase.from('timetable').select('subject_id, weekday').eq('user_id', userId);
  ...
  fullTimetable.forEach(row => {
    ...
    byWeekday[row.weekday].push(row.subject_id);
  });
  ...
  while (cursor <= end) {
    ...
    const subjectIds = byWeekday[dow] || [];
    if (dow !== 0 && subjectIds.length) {
      daysMarked++;
      subjectIds.forEach(subjectId => payloads.push({ user_id: userId, subject_id: subjectId, date: iso, status }));
    }
