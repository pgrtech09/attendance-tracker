...
import { subjectWiseStats, todayLocalISO } from './attendance-calc.js';
...
document.getElementById('restoreInput').addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (Array.isArray(backup.attendance) && backup.attendance.length) {
      // Older backups (or rows missing timetable_id) need it resolved before
      // upserting, since the attendance table's uniqueness is now keyed by
      // (user, timetable period, date) rather than just (user, subject, date).
      const { data: currentTimetable } = await supabase.from('timetable').select('id, subject_id, weekday').eq('user_id', userId);
      const byWeekdaySubject = {};
      (currentTimetable || []).forEach(t => {
        const key = `${t.weekday}:${t.subject_id}`;
        if (!byWeekdaySubject[key]) byWeekdaySubject[key] = t.id;
      });

      const rows = backup.attendance
        .map(r => {
          let timetableId = r.timetable_id;
          if (!timetableId) {
            const weekday = new Date(r.date + 'T00:00:00').getDay();
            timetableId = byWeekdaySubject[`${weekday}:${r.subject_id}`];
          }
          if (!timetableId) return null; // no matching period found — skip rather than guess wrong
          return { user_id: userId, subject_id: r.subject_id, timetable_id: timetableId, date: r.date, status: r.status };
        })
        .filter(Boolean);

      if (rows.length) {
        const { error } = await supabase.from('attendance').upsert(rows, { onConflict: 'user_id,timetable_id,date' });
        if (error) throw error;
      }
      if (rows.length < backup.attendance.length) {
        toast(`${backup.attendance.length - rows.length} record(s) skipped — no matching timetable period found`);
      }
    }
...
document.getElementById('exportPdfBtn').addEventListener('click', async () => {
  const [{ data: attendance }, { data: fullTimetableForExport }] = await Promise.all([
    supabase.from('attendance').select('*').eq('user_id', userId),
    supabase.from('timetable').select('*').eq('user_id', userId)
  ]);
  const stats = subjectWiseStats(attendance || [], subjectsCache, fullTimetableForExport || []);
...
// ---------------- Bulk mark a date range (e.g. a festival week) ----------------
const todayISOForBulk = todayLocalISO();
document.getElementById('bulkFrom').max = todayISOForBulk;
document.getElementById('bulkTo').max = todayISOForBulk;

document.getElementById('bulkMarkBtn').addEventListener('click', async () => {
  const from = document.getElementById('bulkFrom').value;
  const to = document.getElementById('bulkTo').value;
  const status = document.getElementById('bulkStatus').value;
  const resultEl = document.getElementById('bulkMarkResult');

  if (!from || !to) { toast('Pick both a start and end date'); return; }
  if (from > to) { toast('"From" date must be before "To" date'); return; }
  if (to > todayISOForBulk) { toast("Can't mark attendance for future dates"); return; }

  const { data: fullTimetable } = await supabase.from('timetable').select('id, subject_id, weekday').eq('user_id', userId);
  if (!fullTimetable || fullTimetable.length === 0) { toast('No timetable found'); return; }
  const byWeekday = {};
  ...
  while (cursor <= end) {
    ...
    // to UTC first, which silently shifts the date back a day for anyone in
    // a timezone ahead of UTC (e.g. IST, UTC+5:30).
    const iso = `${cursor.getFullYear()}-${String(cursor.getMonth() + 1).padStart(2, '0')}-${String(cursor.getDate()).padStart(2, '0')}`;
    const dow = cursor.getDay();
    const periods = byWeekday[dow] || [];
    if (dow !== 0 && periods.length) {
      daysMarked++;
      periods.forEach(t => payloads.push({ user_id: userId, subject_id: t.subject_id, timetable_id: t.id, date: iso, status }));
    }
    cursor.setDate(cursor.getDate() + 1);
  }

  if (payloads.length === 0) { toast('No scheduled classes found in that range'); return; }
  ...
  btn.disabled = false; btn.textContent = 'Apply to Range';

  if (error) {
    toast('Failed to apply — please try again');
  } else {
    resultEl.textContent = `Marked ${daysMarked} day(s), ${payloads.length} class(es) as ${status === 'holiday' ? 'Holiday' : 'Exam Day'}.`;
    toast('Bulk marking applied ✓');
  }
});
