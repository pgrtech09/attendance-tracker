...
// See app.js `localISODate` for why this exists: `date.toISOString()` first
// converts to UTC, which rolls local dates backward for any timezone ahead
// of UTC (e.g. IST). Kept as a local copy so this module stays dependency-free.
...
// Counts how many periods of `subjectId` the timetable actually scheduled
// between startISO and endISO (inclusive), one count per matching weekday
// occurrence — e.g. if MSF has 2 periods on Thursday, each Thursday in range
// adds 2. This is the "should have happened by now" number, independent of
// whether attendance was ever marked for those classes.
export function scheduledClassCount(subjectId, timetableRows, startISO, endISO) {
  if (!startISO || !endISO) return 0;
  const rowsBySubject = timetableRows.filter(r => r.subject_id === subjectId);
  if (!rowsBySubject.length) return 0;
  const byWeekday = {};
  for (const row of rowsBySubject) {
    byWeekday[row.weekday] = (byWeekday[row.weekday] || 0) + 1;
  }
  ...
  while (cursor <= end) {
    count += byWeekday[cursor.getDay()] || 0;
    cursor.setDate(cursor.getDate() + 1);
  }
  return count;
}

// Subject-wise stats enriched with the timetable's "should have happened by
// now" count, so a gap between Scheduled and (Present+Absent+Holiday+Exam)
// surfaces as classes you simply never went back and marked — invisible in
// the plain present/absent tally alone.
export function subjectWiseStatsWithSchedule(records, subjects, timetableRows, startISO, todayISO) {
  return subjectWiseStats(records, subjects).map(s => {
    const scheduled = scheduledClassCount(s.subject.id, timetableRows, startISO, todayISO);
    const loggedTotal = s.present + s.absent + s.holiday + s.exam;
    const unmarked = Math.max(0, scheduled - loggedTotal);
    return { ...s, scheduled, unmarked };
  });
}
  const bySubject = {};
...
export function remainingWorkingDaysInRange(startISO, endISO, holidayDates, todayISO = localISODate(new Date())) {
