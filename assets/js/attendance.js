...
import { initShell, toast, queueOfflineWrite, localISODate, cachedTable } from './app.js';
...
async function loadSubjectsAndSettings() {
  const [{ data: subjects }, { data: attendance, fromCache }, { data: settings }] = await Promise.all([
  ...
  if (fromCache) toast('Offline — showing last saved data. New marks will sync later.');
...
async function loadDayAndStamps(dateISO) {
  ...
  const { data: timetable } = await cachedTable(
    `timetable_dow${weekday}_${userId}`,
...
async function markWholeDay(dateISO, status) {
  ...
  const alreadyAllSet = timetable.every(t => {
    const rec = attendanceCache.find(a => a.subject_id === t.subject_id && a.date === dateISO);
    return rec?.status === status;
  });

  if (alreadyAllSet) {
    timetable.forEach(t => {
      const idx = attendanceCache.findIndex(a => a.subject_id === t.subject_id && a.date === dateISO);
      if (idx >= 0) attendanceCache.splice(idx, 1);
    });
    if (!navigator.onLine) {
      timetable.forEach(t => queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, subject_id: t.subject_id, date: dateISO } }));
      toast('Cleared — will sync when online');
    } else {
      const { error } = await supabase.from('attendance').delete()
        .eq('user_id', userId).eq('date', dateISO).in('subject_id', timetable.map(t => t.subject_id));
      if (error) { toast('Could not clear — queued for retry'); timetable.forEach(t => queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, subject_id: t.subject_id, date: dateISO } })); }
...
async function markAttendance(subjectId, date, status, cardEl) {
  ...
  if (alreadySet) {
    ...
    if (!navigator.onLine) {
      queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, subject_id: subjectId, date } });
      toast('Cleared — will sync when online');
    } else {
      const { error } = await supabase.from('attendance').delete()
        .eq('user_id', userId).eq('subject_id', subjectId).eq('date', date);
      if (error) { toast('Could not clear — queued for retry'); queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, subject_id: subjectId, date } }); }
