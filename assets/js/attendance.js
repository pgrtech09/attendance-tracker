...
import { subjectWiseStats, averageSubjectPercentage, targetProjection, todayLocalISO } from './attendance-calc.js';
...
async function loadSubjectsAndSettings() {
  const [{ data: subjects }, { data: attendance }, { data: settings }, { data: timetableAll }] = await Promise.all([
  ...
  timetableAllCache = timetableAll || [];
...
async function loadDayAndStamps(dateISO) {
  ...
  // order by period_order so a subject scheduled twice in a day (e.g. MSF on
  // Thursdays) always shows morning-then-afternoon in a stable order.
...
// Each card is keyed by the specific timetable_id (period), not just the
// subject — this is what allows the same subject to appear twice in one day
// (different period, different card, independent status) instead of two
// cards silently sharing and overwriting a single stored record.
function renderStamps(timetable, dateISO) {
  ...
  container.innerHTML = timetable.map(t => {
    const record = attendanceCache.find(a => a.timetable_id === t.id && a.date === dateISO);
    ...
    return `
      <div class="card" style="margin-bottom:14px;" data-timetable-id="${t.id}">
        ...
        <div class="stamp-row">
          <button class="stamp ${status === 'present' ? 'selected' : ''}" data-status="present" data-timetable="${t.id}" data-subject="${t.subject_id}">Present</button>
          <button class="stamp ${status === 'absent' ? 'selected' : ''}" data-status="absent" data-timetable="${t.id}" data-subject="${t.subject_id}">Absent</button>
        </div>
      </div>`;
  }).join('');

  container.querySelectorAll('.stamp').forEach(btn => {
    btn.addEventListener('click', () => markAttendance(btn.dataset.timetable, btn.dataset.subject, dateISO, btn.dataset.status, btn.closest('.card')));
  });
}

// Highlights the whole-day Holiday/Exam buttons only if every scheduled
// PERIOD that day (not just every distinct subject) already carries that status.
function updateDayButtonHighlight(timetable, dateISO) {
  ...
  const records = timetable.map(t => attendanceCache.find(a => a.timetable_id === t.id && a.date === dateISO));
...
async function markWholeDay(dateISO, status) {
  ...
  const weekday = new Date(dateISO + 'T00:00:00').getDay();
  const { data: timetable } = await supabase.from('timetable').select('id, subject_id')
    .eq('user_id', userId).eq('weekday', weekday);
  if (!timetable || timetable.length === 0) { toast('No classes scheduled on this day'); return; }

  // If the whole day is already marked with this status, clicking again clears it.
  const alreadyAllSet = timetable.every(t => {
    const rec = attendanceCache.find(a => a.timetable_id === t.id && a.date === dateISO);
    return rec?.status === status;
  });

  if (alreadyAllSet) {
    timetable.forEach(t => {
      const idx = attendanceCache.findIndex(a => a.timetable_id === t.id && a.date === dateISO);
      if (idx >= 0) attendanceCache.splice(idx, 1);
    });
    if (!navigator.onLine) {
      timetable.forEach(t => queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, timetable_id: t.id, date: dateISO } }));
      toast('Cleared — will sync when online');
    } else {
      const { error } = await supabase.from('attendance').delete()
        .eq('user_id', userId).eq('date', dateISO).in('timetable_id', timetable.map(t => t.id));
      if (error) { toast('Could not clear — queued for retry'); timetable.forEach(t => queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, timetable_id: t.id, date: dateISO } })); }
      else toast('Whole day cleared to Unmarked');
    }
  ...
  const payloads = timetable.map(t => ({ user_id: userId, subject_id: t.subject_id, timetable_id: t.id, date: dateISO, status }));
  for (const payload of payloads) {
    const idx = attendanceCache.findIndex(a => a.timetable_id === payload.timetable_id && a.date === dateISO);
  ...
  if (!navigator.onLine) {
    ...
    const { error } = await supabase.from('attendance').upsert(payloads, { onConflict: 'user_id,timetable_id,date' });
...
async function markAttendance(timetableId, subjectId, date, status, cardEl) {
  if (date > todayISO) {
    toast("Can't mark attendance for a future date");
    return;
  }
  const existingIdx = attendanceCache.findIndex(a => a.timetable_id === timetableId && a.date === date);
  const alreadySet = existingIdx >= 0 && attendanceCache[existingIdx].status === status;

  // Clicking an already-selected status undoes it, back to Unmarked.
  if (alreadySet) {
    cardEl.querySelectorAll('.stamp').forEach(b => b.classList.remove('selected'));
    const pill = cardEl.querySelector('.status-pill');
    if (pill) { pill.className = 'status-pill status-unmarked'; pill.textContent = 'Unmarked'; }
    attendanceCache.splice(existingIdx, 1);

    if (!navigator.onLine) {
      queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, timetable_id: timetableId, date } });
      toast('Cleared — will sync when online');
    } else {
      const { error } = await supabase.from('attendance').delete()
        .eq('user_id', userId).eq('timetable_id', timetableId).eq('date', date);
      if (error) { toast('Could not clear — queued for retry'); queueOfflineWrite({ table: 'attendance', action: 'delete', payload: { user_id: userId, timetable_id: timetableId, date } }); }
      else toast('Cleared to Unmarked');
    }
    renderTargetPanel();
    renderSubjectList();
    return;
  ...
  const payload = { user_id: userId, subject_id: subjectId, timetable_id: timetableId, date, status };
  cardEl.querySelectorAll('.stamp').forEach(b => b.classList.toggle('selected', b.dataset.status === status));
  const pill = cardEl.querySelector('.status-pill');
  if (pill) { pill.className = `status-pill status-${status}`; pill.textContent = STATUS_META[status].label; }
  ...
  if (!navigator.onLine) {
    ...
    const { error } = await supabase.from('attendance').upsert(payload, { onConflict: 'user_id,timetable_id,date' });
...
function renderTargetPanel() {
  const stats = subjectWiseStats(attendanceCache, subjectsCache, timetableAllCache);
