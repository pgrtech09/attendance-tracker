...
// Local copy of app.js's localISODate — can't import app.js here since
// app.js imports auth.js (would create a circular import). See app.js for
// why toISOString() alone is wrong for local dates.
...
async function seedDefaultsForUser(userId) {
  ...
  const { error: settingsErr } = await supabase.from('semester_settings').insert({
    user_id: userId,
    start_date: localISODate(semStart),
    end_date: localISODate(semEnd),
